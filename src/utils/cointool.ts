...
export class CoinTool {
    ...
    static makeTran(utxos, targetaddr, assetid, sendcount) {
        ...
        for (var i = 0; i < us.length; i++) {
            ...
            input.hash = Helper.UintHelper.hexToBytes(us[i].txid).reverse();
            input.index = us[i].n;
        ...
        if (count.compareTo(sendcount) >= 0)//输入大于等于输出
        {
            ...
            if (sendcount.compareTo(Neo.Fixed8.Zero) > 0) {
                ...
                //资产类型
                output.assetId = Helper.UintHelper.hexToBytes(assetid).reverse();
                //交易金额
                output.value = sendcount;
                //目的账户
                output.toAddress = Helper.Helper.GetPublicKeyScriptHash_FromAddress(targetaddr);
                //添加转账交易
            ...
            if (change.compareTo(Neo.Fixed8.Zero) > 0) {
                ...
                //找零地址设置为自己
                outputchange.toAddress = Helper.Helper.GetPublicKeyScriptHash_FromAddress(scraddr);
                //设置找零额度
                outputchange.value = change;
                //找零资产类型
                outputchange.assetId = Helper.UintHelper.hexToBytes(assetid).reverse();
                //添加找零交易
    ...
    static async nep5Transaction(address, tatgeraddr, asset, amount)
    {
        ...
        var scriptaddress = Helper.UintHelper.hexToBytes(asset).reverse();
