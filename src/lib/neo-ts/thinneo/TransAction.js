...
const Fixed8_1 = require("../neo/Fixed8");
const BinaryWriter_1 = require("../neo/IO/BinaryWriter");
const Sha256_1 = require("../neo/Cryptography/Sha256");
const MemoryStream_1 = require("../neo/IO/MemoryStream");
var TransactionType;
...
class Witness {
    get Address() {
        var hash = AccountHelper_1.Helper.GetScriptHashFromScript(this.VerificationScript);
        return AccountHelper_1.Helper.GetAddressFromScriptHash(hash);
    }
}
...
class InvokeTransData {
    ...
    Deserialize(trans, reader) {
        ...
        if (trans.version >= 1) {
            this.gas = new Fixed8_1.Fixed8(reader.readUint64());
        }
    }
}
exports.InvokeTransData = InvokeTransData;
class ClaimTransData {
    Serialize(trans, writer) {
...
exports.ClaimTransData = ClaimTransData;
class MinerTransData {
    Serialize(trans, writer) {
...
exports.MinerTransData = MinerTransData;
class Transaction {
    ...
    Deserialize(ms) {
        ...
        for (var i = 0; i < countOutputs; i++) {
            ...
            var value = new Fixed8_1.Fixed8(ms.readUint64());
            var arr = ms.readBytes(20);
    ...
    GetMessage() {
        var ms = new MemoryStream_1.MemoryStream();
        var writer = new BinaryWriter_1.BinaryWriter(ms);
        this.SerializeUnsigned(writer);
        var arr = ms.toArray();
    ...
    AddWitness(signdata, pubkey, addrs) {
        {
            ...
            var bsign = AccountHelper_1.Helper.VerifySignature(msg, signdata, pubkey);
            if (bsign == false)
                throw new Error("wrong sign");
            var addr = AccountHelper_1.Helper.GetAddressFromPublicKey(pubkey);
            if (addr != addrs)
                throw new Error("wrong script");
        }
        var vscript = AccountHelper_1.Helper.GetAddressCheckScriptFromPublicKey(pubkey);
        var sb = new ScriptBuilder_1.ScriptBuilder();
        sb.EmitPushBytes(signdata);
        var iscript = sb.ToArray();
        this.AddWitnessScript(vscript, iscript);
    }
    AddWitnessScript(vscript, iscript) {
        var scripthash = AccountHelper_1.Helper.GetScriptHashFromScript(vscript);
    ...
    GetHash() {
        var msg = this.GetMessage();
        var data = Sha256_1.Sha256.computeHash(msg);
        data = Sha256_1.Sha256.computeHash(data);
