...
class BigInteger {
    constructor(value) {
        ...
        if (typeof value === "number") {
            ...
            if (parts.man.equals(Uint64_1.Uint64.Zero) || parts.exp <= -64)
                return;
            if (parts.exp <= 0) {
    ...
    static getDoubleParts(dbl) {
        ...
        let result = {
            sign: 1 - ((uu[1] >>> 30) & 2),
            man: new Uint64_1.Uint64(uu[0], uu[1] & 0x000FFFFF),
            exp: (uu[1] >>> 20) & 0x7FF,
            fFinite: true
        };
        if (result.exp == 0) {
            if (!result.man.equals(Uint64_1.Uint64.Zero))
                result.exp = -1074;
        }
            ...
            result.man = result.man.or(new Uint64_1.Uint64(0, 0x00100000));
            result.exp -= 1075;
        }
        return result;
    }
    ...
    static random(bitLength, randomStr) {
        ...
        if (randomStr == null) {
            ...
            bytes = UintHelper.hexToBytes(randomStr.slice(0, bitLength));
        }
