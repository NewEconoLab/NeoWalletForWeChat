...
class Fixed8 {
    ...
    static get MaxValue() { return _max || (_max = new Fixed8(new Uint64_1.Uint64(0xffffffff, 0x7fffffff))); }
    static get MinusOne() { return _minus || (_minus = new Fixed8(new Uint64_1.Uint64(0xffffffff, 0xffffffff))); }
    static get MinValue() { return _min || (_min = new Fixed8(Uint64_1.Uint64.MinValue)); }
    static get One() { return _one || (_one = Fixed8.fromNumber(1)); }
    static get Satoshi() { return _satoshi || (_satoshi = new Fixed8(new Uint64_1.Uint64(1))); }
    ...
    static fromNumber(value) {
        ...
        if (value >= 0x8000000000000000)
            throw new RangeError();
        let array = new Uint32Array((new BigInteger_1.BigInteger(value)).toUint8Array(true, 8).buffer);
        return new Fixed8(new Uint64_1.Uint64(array[0], array[1]));
    }
    getData() {
        return this.data;
    }
    ...
    static parse(str) {
        ...
        if (digits > 8)
            ...
            for (let i = digits; i < 8; i++)
                str += '0';
        let bi = BigInteger_1.BigInteger.parse(str);
        if (bi.bitLength() > 64)
            throw new RangeError();
        return new Fixed8(Uint64_1.Uint64.parse(bi.toUint8Array(true, 8).buffer));
    }
    subtract(other) {
