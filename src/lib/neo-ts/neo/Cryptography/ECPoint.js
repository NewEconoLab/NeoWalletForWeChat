...
class ECPoint {
            ...
            case 0x03:
                {
                    ...
                    let X1 = BigInteger_1.BigInteger.fromUint8Array(encoded.subarray(1), 1, false);
            ...
            case 0x07:
                {
                    ...
                    let X1 = BigInteger_1.BigInteger.fromUint8Array(encoded.subarray(1, 1 + expectedLength), 1, false);
                    let Y1 = BigInteger_1.BigInteger.fromUint8Array(encoded.subarray(1 + expectedLength), 1, false);
                    p = new ECPoint(new ECFieldElement_1.ECFieldElement(X1, curve), new ECFieldElement_1.ECFieldElement(Y1, curve), curve);
                    break;
                }
            default:
    ...
    static decompressPoint(yTilde, X1, curve) {
        let x = new ECFieldElement_1.ECFieldElement(X1, curve);
        ...
        if (bit0 != yTilde) {
            beta = new ECFieldElement_1.ECFieldElement(curve.Q.subtract(betaValue), curve);
        }
    ...
    static multiply(p, n) {
        let k = n instanceof Uint8Array ? BigInteger_1.BigInteger.fromUint8Array(n, 1, false) : n;
        if (p.isInfinity())
            return p;
        if (k.isZero())
    ...
    twice() {
        ...
        let TWO = new ECFieldElement_1.ECFieldElement(new BigInteger_1.BigInteger(2), this.curve);
        let THREE = new ECFieldElement_1.ECFieldElement(new BigInteger_1.BigInteger(3), this.curve);
    ...
    static windowNaf(width, k) {
        ...
        while (k.sign() > 0) {
            if (!k.isEven()) {
                let remainder = BigInteger_1.BigInteger.remainder(k, pow2wB);
                if (remainder.testBit(width - 1)) {
                    wnaf[i] = BigInteger_1.BigInteger.subtract(remainder, pow2wB).toInt32();
                }
                else {
