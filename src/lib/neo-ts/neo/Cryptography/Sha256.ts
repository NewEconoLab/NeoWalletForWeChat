namespace Neo.Cryptography
{
    export class Sha256
    {
        ...
        private static ROTR(n: number, x: number) { return (x >>> n) | (x << (32 - n)); }

        // Logical functions [§4.1.2].
        private static Σ0(x: number) { return Sha256.ROTR(2, x) ^ Sha256.ROTR(13, x) ^ Sha256.ROTR(22, x); }
        private static Σ1(x: number) { return Sha256.ROTR(6, x) ^ Sha256.ROTR(11, x) ^ Sha256.ROTR(25, x); }
        private static σ0(x: number) { return Sha256.ROTR(7, x) ^ Sha256.ROTR(18, x) ^ (x >>> 3); }
        private static σ1(x: number) { return Sha256.ROTR(17, x) ^ Sha256.ROTR(19, x) ^ (x >>> 10); }
        private static Ch(x: number, y: number, z: number) { return (x & y) ^ (~x & z); }
        private static Maj(x: number, y: number, z: number) { return (x & y) ^ (x & z) ^ (y & z); }
    }
}
